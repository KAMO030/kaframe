...
public abstract class AbstractApplicationContext implements ApplicationContext, BeanDefinitionRegistry {
    ...
    protected final Map<Class, FactoryBean> factoryBeans;
    ...
    public <T> T getBeanByFactory(Class<T> type) {
        FactoryBean<T> factoryBean = factoryBeans.get(type);
        if (factoryBean == null) {
            return null;
        }
        T object = factoryBean.getObject();
        return object;
    }
