package com.kamo.cglib;

import com.kamo.cglib.asm.AsmProxyClassWriter;
import com.kamo.cglib.asm.AsmUtil;
...
public class ProxyClass {
    ...
    public static <T> T newProxyInstance(ClassLoader classLoader, Class<T> superclass, boolean isWriteFile, InvocationHandler handler) {
        ...
        Class proxyClass = getProxyClass(classLoader, superclass, isWriteFile);

        try {
            T proxy = (T) proxyClass.getConstructor(InvocationHandler.class).newInstance(handler);
            return proxy;
